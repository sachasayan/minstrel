        ...
        {!isFreshNewProject ? (
          <>
            <motion.div
              ...
              className={cn('fixed top-4 right-4 z-50 flex items-center gap-2', isProjectEmpty && 'pointer-events-none')}
            >
              <ProjectBar />
            ...
            <motion.main
              animate={{ opacity: isProjectEmpty ? 0 : 1 }}
              transition={{ duration: 0.5 }}
              className={cn('relative flex flex-1 min-w-0 h-full overflow-hidden min-h-0', isProjectEmpty && 'pointer-events-none')}
            >
              <div className="min-w-0 flex-1">
                {appState.activeView === 'project/editor' && editorContent ? (
                  <StoryViewer key={activeProject?.projectPath} activeSection={appState.activeSection} content={editorContent.content} />
                ) : null}
              </div>
