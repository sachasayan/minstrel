...
const SettingsModal = ({ open, onClose }: SettingsModalProps): ReactNode => {
  ...
  const currentModelOptions = MODEL_OPTIONS_BY_PROVIDER[settings.provider || 'google'] || MODEL_OPTIONS_BY_PROVIDER.google
  ...
  useEffect(() => {
    ...
    const hasAnySetting = Boolean(settings.provider) || Boolean(settings.workingRootDirectory) || Boolean(settings.highPreferenceModelId) || Boolean(settings.lowPreferenceModelId)
    ...
    return (
      ...
      >
        ...
        {keyValidationStatus === 'invalid' && (
          <>
            <CircleX className="h-3 w-3" /> Invalid
          </>
        )}
      </div>
    )
  ...
  const handleWritingSampleKeyDown: React.KeyboardEventHandler<HTMLTextAreaElement> = (event) => {
    const isPasteShortcut = (event.key.toLowerCase() === 'v' && (event.metaKey || event.ctrlKey)) || (event.key === 'Insert' && event.shiftKey)

    const isNavigationKey = ['Tab', 'Escape', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Home', 'End', 'PageUp', 'PageDown'].includes(event.key)
    ...
    >
      <DialogContent className="sm:max-w-3xl p-0 gap-0 overflow-hidden">
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab.id ? 'bg-background text-foreground shadow-sm border' : 'text-muted-foreground hover:text-foreground hover:bg-accent/60'
                )}
              >
                {tab.icon}
                {tab.label}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            {activeTab === 'ai' && (
              <>
                ...
                <Section title="Models" icon={<Sparkles className="h-3.5 w-3.5" />}>
                  <SettingRow label="High Preference Model" description="Used for complex tasks like outlining and writing.">
                  ...
                  <SettingRow label="Low Preference Model" description="Used for simpler tasks like routing and critique.">
            ...
            {activeTab === 'personalization' && (
              <Section title="Writing Style" icon={<PenTool className="h-3.5 w-3.5" />}>
                <SettingRow label="Writing Sample" description="Paste a sample of your writing here so Minstrel can analyze it and mimic your style while drafting chapters.">
                ...
                <SettingRow label="Style Analysis" description="This generated style description is what Minstrel will inject into the writing prompt.">
                  <Textarea
            ...
            {activeTab === 'workspace' && (
              <Section title="Project Directory" icon={<FolderOpen className="h-3.5 w-3.5" />}>
                <p className="text-sm text-muted-foreground -mt-1">The folder where Minstrel reads and saves your projects.</p>
                <Button variant="outline" onClick={selectFolder} className="w-full justify-start gap-2">
                  <FolderOpen className="h-4 w-4 shrink-0" />
                  Choose folder…
                </Button>
                {settings.workingRootDirectory ? (
                  <div className="rounded-md bg-muted/60 px-3 py-2 text-xs font-mono text-muted-foreground break-all">{settings.workingRootDirectory}</div>
                ) : (
                  <p className="text-xs text-muted-foreground italic">No folder selected — using system default.</p>
                )}
              </Section>
            )}
          </div>
        ...
        <div className="px-6 py-3 border-t flex items-center justify-between bg-accent/10">
          <span className="text-xs text-muted-foreground">{lastSaved ? `Saved ${lastSaved.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : 'Changes saved automatically'}</span>
