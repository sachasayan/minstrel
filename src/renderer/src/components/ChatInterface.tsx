...
import ChatMessage from '@/components/ChatMessage' // Import the extracted ChatMessage component
import ChatLoadingIndicator from '@/components/ChatLoadingIndicator' // Import the extracted component

import useChatScroll from '@/hooks/useChatScroll' // Import the new hook
...
const ChatInterface = forwardRef<HTMLDivElement, ChatInterfaceProps>(({ expanded, setExpanded }, ref) => {
  const { chatHistory, pendingChat, streamingMessage } = useSelector((state: RootState) => selectChat(state))
  ...
  useChatScroll(lastMessageRef, [chatHistory, streamingMessage]) // Use the custom hook
            ...
            >
              <div className="flex-1 overflow-y-auto p-4" ref={chatContainerRef}>
                {chatHistory.map((msg, index) => (
                  <ChatMessage
                    key={index}
                    msg={msg}
                    isLastMessage={index === chatHistory.length - 1 && !pendingChat}
                ...
                {pendingChat && !streamingMessage && <ChatLoadingIndicator />}
                {/* Robot cursor at the end of the chat */}
              ...
              <div className="border-t p-2 flex items-center">
                <input
                  ...
                  disabled={pendingChat} // Disable input while pending/streaming
                />
