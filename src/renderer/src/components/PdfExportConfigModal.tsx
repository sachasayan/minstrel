...
// --- Component ---
const PdfExportConfigModal: React.FC<PdfExportConfigModalProps> = ({ children, onExport, triggerTooltip }) => {
  ...
  const currentPreviewConfig = useMemo((): PdfExportConfig | null => {
    ...
    const paperSizeForPdf = selectedSizeOption?.dimensions ? (selectedSizeOption.dimensions as [number, number]) : (selectedSizeOption?.value as 'A4' | 'Letter')
  ...
  const handleExportClick = () => {
    ...
    const finalPaperSize = selectedSizeOption?.dimensions ? (selectedSizeOption.dimensions as [number, number]) : (selectedSizeOption?.value as 'A4' | 'Letter')
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-4xl h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>Configure the appearance of your exported PDF document.</DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-grow grid grid-cols-1 md:grid-cols-3 gap-6 overflow-hidden">
          ...
          <div className="md:col-span-2 h-full overflow-hidden bg-muted rounded border border-border">
            {activeProject && currentPreviewConfig ? (
              ...
              <div className="flex items-center justify-center h-full text-muted-foreground">{!activeProject ? 'No active project for preview.' : 'Select valid options for preview.'}</div>
            )}
          </div>
          {/* End Flex Layout */}
        </div>
