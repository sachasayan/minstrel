...
const CoverSelectionModal: React.FC<CoverSelectionModalProps> = ({ open, onOpenChange, onSelectCover }) => {
  ...
  const filteredCovers = useMemo(() => {
    ...
    return bookCovers.filter((cover) => cover.categoryName.startsWith(selectedGenreLabel))
  }, [selectedGenre])
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-4xl h-[80vh] flex flex-col p-0 gap-0">
        <DialogHeader className="p-6 pb-2 border-b">
          ...
          <DialogDescription>Choose a pre-designed cover or upload your own.</DialogDescription>
        </DialogHeader>
          ...
          <Tabs value={selectedTab} onValueChange={setSelectedTab} className="h-full flex flex-col">
            ...
            <TabsContent value="gallery" className="flex-grow overflow-hidden flex flex-col data-[state=inactive]:hidden p-6 pt-4 h-full">
              ...
              <div className="flex items-center gap-4 mb-4 shrink-0">
                <Label htmlFor="genre-select" className="shrink-0">
                  Filter by Genre:
                </Label>
              ...
              <div className="flex-grow overflow-y-auto border rounded-md p-4 bg-muted/20">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  ...
                  {filteredCovers.length === 0 && <div className="col-span-full h-40 flex items-center justify-center text-muted-foreground">No covers found for this genre.</div>}
                </div>
              </div>
            </TabsContent>
                ...
                {uploadPreview ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">Supports JPG, PNG, WebP (Max {MAX_IMAGE_SIZE_MB}MB)</p>
                    </div>
                    <Button onClick={() => fileInputRef.current?.click()}>Select File</Button>
                    <input ref={fileInputRef} type="file" className="hidden" accept={ALLOWED_IMAGE_TYPES.join(',')} onChange={handleFileChange} />
                  </div>
                )}
              </div>
            </TabsContent>
