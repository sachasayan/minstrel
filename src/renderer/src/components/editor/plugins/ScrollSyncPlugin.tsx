...
export function ScrollSyncPlugin({ activeSection, onSectionChange, containerRef, instantInitialScroll = false }: ScrollSyncPluginProps): null {
  ...
  useEffect(() => {
    ...
    editor.getEditorState().read(() => {
      ...
      const headingNodes = root
        .getChildren()
        .filter((node): node is ChapterHeadingNode => node instanceof HeadingNode && node.getTag() === 'h1' && $isChapterHeadingNode(node) && !!node.getChapterId())

      const targetNode = headingNodes.find((node, index) => node.getChapterId() === activeSection.chapterId || index === activeSection.index)
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        const now = Date.now()
        if (candidate.sectionKey === lastDispatchedSectionKey.current && now - lastDispatchAt.current < 150) {
          return
        }
