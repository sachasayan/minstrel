...
export function DashboardRibbon() {
  ...
  return (
    <div className={`w-full overflow-hidden transition-all ${hasVisibleSections ? 'mb-12' : 'mb-0 max-h-0'}`} data-state={hasVisibleSections ? 'expanded' : 'collapsed'} id="project-overview">
      <div className="grid grid-flow-dense auto-rows-[minmax(220px,_auto)] grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <CardContent className="p-0">
              <ChartContainer className="h-44 w-full" config={{}}>
                <LineChart data={dialogueCountData}>
                  ...
                  <YAxis hide />
                  <ChartTooltip content={<ChartTooltipContent hideLabel />} />
                  {Object.keys(dialogueCountData[0] || {})
                    .filter((k) => k !== 'chapter')
                    .map((name, i) => (
        ...
        {activeProject.expertSuggestions.map((suggestion, index) => (
          <Card key={index} className="min-h-[220px] border-none bg-muted/20 shadow-sm md:col-span-1 xl:col-span-1">
