...
const createStyles = (config: PdfExportConfig) =>
  StyleSheet.create({
    page: {
...
export const ProjectPdfDocument: React.FC<ProjectPdfProps> = ({ project, config }) => {
  ...
  const getPaperSize = (): PdfPageSize => {
    // Use the specific type alias
    ...
    <Document>
      ...
      <Page size={paperSize} style={styles.titlePage}>
        <View style={styles.titlePageView}>
          {coverDataUrl && <Image style={styles.coverImage} src={coverDataUrl} />}
