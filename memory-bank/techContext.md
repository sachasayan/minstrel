...
- `Intro` uses `fileService` (via `fetchProjects`) to list projects (displaying covers via `ProjectLibrary`) and triggers the full project load via Redux actions handled by listeners. Contains a button that dispatches `setActiveView('settings')`.
- `ProjectOverview` displays the loaded project data (including chat) from the Redux store (`projectsSlice`, `chatSlice`). Renders different views (`MarkdownViewer`, `NovelDashboard`, `ProjectParameters`) based on `appState.activeView`. Includes `AppSidebar` and `ChatInterface`.
